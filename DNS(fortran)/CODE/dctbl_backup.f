...
C     DNS OF TURBULENT BOUNDARY LAYER 
...
c     Time history file  : FILE I/O NUMBER BEGINS FROM 21
      IF (ITYPE.EQ.0) THEN  ! ONLY FOR INFLOW GENERATION
      open(21,file='OUTPUT/Cf.plt',status='unknown') 
      open(24,file='OUTPUT/alert.txt',status='unknown') 
      ENDIF
      ...
      open(22,file='OUTPUT/Flow.plt',status='unknown') 
