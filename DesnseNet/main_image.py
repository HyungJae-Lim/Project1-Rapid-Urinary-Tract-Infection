...
def get_images(directory):
    ...
    for labels in os.listdir(
        ...
        for image_file in os.listdir(
            ...
            image = io.imread(directory + labels + r"/" + image_file)
            #            image = np.asarray(image)
            image = np.array(image, dtype=np.float32)
            image = image / 65536.0
            tif_mean = np.mean(image)
            #            sub_mean = image - tif_mean
            tif_max = np.max(image)
            tif_min = np.min(image)

            image = (image - tif_min) / (tif_max - tif_min)

            #            stand = preprocessing.scale(image)

            #            image = cv2.imread(directory+labels+r'/'+image_file) #Reading the image (OpenCV)
            #            image = cv2.resize(image,(300,300)) #Resize the image, Some images are different sizes. (Resizing is very Important)
    ...
    return shuffle(
        Images, Labels, random_state=817328462
...
def get_images_test(directory):
    ...
    for labels in os.listdir(
        ...
        for image_file in os.listdir(
            ...
            image_test = io.imread(directory + labels + r"/" + image_file)
            #            image = np.asarray(image)
            image_test = np.array(image_test, dtype=np.float32)
            image_test = image_test / 65535.0
            tif_mean_test = np.mean(image_test)
            #            sub_mean_test = image_test - tif_mean_test
            tif_max_test = np.max(image_test)
            tif_min_test = np.min(image_test)

            image_test = (image_test - tif_min_test) / (tif_max_test - tif_min_test)

            #            stand_test = preprocessing.scale(image_test)
    ...
    return shuffle(
        Images_test, Labels_test, random_state=817328462
    )  # Shuffle the dataset you just prepared.
...
Images, Labels = get_images(
    "/data/hjim/Urine/input/train2/"
)  # Extract the training images from the folders.
Images_test, Labels_test = get_images_test("/data/hjim/Urine/input/val/")
