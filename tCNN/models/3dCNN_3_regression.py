...
class tCNN_3_regression(nn.Module):
    def __init__(self, num_classes,channels_cnn = [256, 256, 256, 256 ,256], hidden_fc = [128, 64]):
        ...
        for idx, channel_input in enumerate(channels_cnn[0:-1]):
            ...
            count = count + 1
            self.first_block.add_module('{}'.format(count), nn.LeakyReLU(negative_slope = 0.1, inplace=True))
            count = count + 1
        ...
        for idx, width_layer in enumerate(size_fc[0:-1]):
            ...
            if idx == size_fc.__len__()-2:
                self.second_block.add_module('{}'.format(count), nn.LeakyReLU(negative_slope = 0.1, inplace=True))
                count = count+1
            else:
                self.second_block.add_module('{}'.format(count),nn.LeakyReLU(negative_slope = 0.1, inplace=True))
                count = count + 1
