...
class tCNN(nn.Module):
    def __init__(self, num_classes,first_channel_num):
        super(tCNN, self).__init__()

        # [[Conv -> RELU]* N -> POOL? ] * M
        self.first_block = nn.Sequential(
            # 1st point conv
            nn.Conv2d(in_channels=first_channel_num, out_channels=32, kernel_size=1, stride=1, padding=0,
                      bias=True),
            ...
            # 3rd point conv
            nn.Conv2d(in_channels=64, out_channels=96, kernel_size=1, stride=1, padding=0,
                      bias=True),
            ...
            # 4th point conv
            nn.Conv2d(in_channels=96, out_channels=128, kernel_size=1, stride=1, padding=0,
                      bias=True),
...
def train_cnn_tomo(model, optimizer, epochs,loader_train, loader_val, batch_size,loss_function):
