...
class tCNN_3(nn.Module):
    def __init__(self, num_classes,channels_cnn = [60, 60, 60, 60, 60, 60, 60, 60, 60, 60], hidden_fc = [512, 256, 128], norm = False):
        ...
        self.first_block.add_module('{}'.format(count), nn.AdaptiveAvgPool2d((1,1)))
        count = count+1
        ...
        for idx, width_layer in enumerate(size_fc[0:-1]):
            ...
            if idx == size_fc.__len__():#-2:
                self.second_block.add_module('{}'.format(count), nn.Softmax(dim=1))
                count = count+1
            else:
        ...
        for idx, m in enumerate(self.first_block.modules()):
            print(idx, '->', m)
        ##print(self.second_block.modules())
        for idx, m in enumerate(self.second_block.modules()):
            print(idx, '->', m)
