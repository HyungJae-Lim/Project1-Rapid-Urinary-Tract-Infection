...
def cam1step1(serial):
    ...
    with Camera(index = serial) as cam:
        ...
        if os.path.isfile("/home/%s/Desktop/variable/exposure4" % name):
            f = open("/home/%s/Desktop/variable/exposure4" % name, 'r+t')
            exposure1 = f.read()
            print("expoure4:", exposure1)
            f.close()
        ...
        while targetExposureReached == False:
            ...
            for frame in range(1,999):
                ...
                if avg == 0:
                    ...
                    cam.ExposureTime = shutterSpeed
                    f = open("/home/%s/Desktop/variable/exposure4" % name, 'w+t')
...
def cam1step2(serial):
    ...
    with Camera(index = serial) as cam:
        ...
        if os.path.isfile("/home/%s/Desktop/variable/exposure4" % name):
            f = open("/home/%s/Desktop/variable/exposure4" % name, 'r+t')
            exposure1 = f.read()
            print("step2_expoure4:", exposure1)
            f.close()
        ...
        f1 = open("/home/%s/Desktop/variable/cam4_xoffset" % name, 'w+t')
        f1.write(str(loc[0]))
        f1.close()

        f2 = open("/home/%s/Desktop/variable/cam4_yoffset" % name, 'w+t')
        f2.write(str(loc[1]))
        f2.close()
        cam.stop()
...
def cam1step3(serial):
    ...
    with Camera(index = serial) as cam:
        ...
        name = os.environ['LOGNAME']
        if os.path.isfile("/home/%s/Desktop/variable/cam4_xoffset" % name):
            f1 = open("/home/%s/Desktop/variable/cam4_xoffset" % name, 'r+t')
            cam1offx = f1.read()
            print("step3_cam4offx:", cam1offx)
            f1.close()

        if os.path.isfile("/home/%s/Desktop/variable/cam4_yoffset" % name):
            f2 = open("/home/%s/Desktop/variable/cam4_yoffset" % name, 'r+t')
            cam1offy = f2.read()
            print("step3_cam4offy:", cam1offy)
            f2.close()
        ...
        if os.path.isfile("/home/%s/Desktop/variable/exposure4_2" % name):
            f = open("/home/%s/Desktop/variable/exposure4_2" % name, 'r+t')
            exposure1 = f.read()
            print("step3_expoure4:", exposure1)
            f.close()
        ...
        while targetExposureReached == False:
            ...
            for frame in range(1,999):
                ...
                if avg == 0:
                    targetExposureReached =True
                    cam.ExposureTime = shutterSpeed
                    f = open("/home/%s/Desktop/variable/exposure4_2" % name, 'w+t')
                    f.write(str(shutterSpeed))
                    f.close()
                    break
                else:
...
def check(serial):
    ...
    with Camera(index = serial) as cam:
        ...
        if os.path.isfile("/home/%s/Desktop/variable/cam4_xoffset" % name):
            f1 = open("/home/%s/Desktop/variable/cam4_xoffset" % name, 'r+t')
            cam1offx = f1.read()
            print("check_cam4offx:", cam1offx)
            f1.close()

        if os.path.isfile("/home/%s/Desktop/variable/cam4_yoffset" % name):
            f2 = open("/home/%s/Desktop/variable/cam4_yoffset" % name, 'r+t')
            cam1offy = f2.read()
            print("check_cam4offy:", cam1offy)
            f2.close()
        ...
        if os.path.isfile("/home/%s/Desktop/variable/exposure4" % name):
            f = open("/home/%s/Desktop/variable/exposure4" % name, 'r+t')
            exposure1 = f.read()
            print("check_expoure4:", exposure1)
            f.close()
...
if __name__=='__main__':
    ...
    cam1step3(3)
    check(3)
    print("cam4:Full ROI calibration finished")
