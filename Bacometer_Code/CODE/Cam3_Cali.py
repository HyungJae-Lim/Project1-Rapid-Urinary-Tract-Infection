        ...
        while targetExposureReached == False:
            ...
            for frame in range(1,999):
                ...
                if avg == 0:
                    ...
                    cam.ExposureTime = shutterSpeed
                    f = open("/home/%s/Desktop/variable/exposure3" % name, 'w+t')
...
def cam1step2(serial):
    ...
    with Camera(index = serial) as cam:
        ...
        f1 = open("/home/%s/Desktop/variable/cam3_xoffset" % name, 'w+t')
        f1.write(str(loc[0]))
        f1.close()

        f2 = open("/home/%s/Desktop/variable/cam3_yoffset" % name, 'w+t')
        f2.write(str(loc[1]))
        f2.close()
        cam.stop()
...
def cam1step3(serial):
    ...
    with Camera(index = serial) as cam:
        ...
        if os.path.isfile("/home/%s/Desktop/variable/exposure3_2" % name):
            f = open("/home/%s/Desktop/variable/exposure3_2" % name, 'r+t')
            exposure1 = f.read()
            print("step3_expoure3:", exposure1)
            f.close()
        ...
        while targetExposureReached == False:
            ...
            for frame in range(1,999):
                ...
                if avg == 0:
                    targetExposureReached =True
                    cam.ExposureTime = shutterSpeed
                    f = open("/home/%s/Desktop/variable/exposure3_2" % name, 'w+t')
                    f.write(str(shutterSpeed))
                    f.close()
                    break
                else:
...
def check(serial):
    ...
    with Camera(index = serial) as cam:
        ...
        if os.path.isfile("/home/%s/Desktop/variable/cam3_xoffset" % name):
            f1 = open("/home/%s/Desktop/variable/cam3_xoffset" % name, 'r+t')
            cam1offx = f1.read()
            print("check_cam3offx:", cam1offx)
            f1.close()

        if os.path.isfile("/home/%s/Desktop/variable/cam3_yoffset" % name):
            f2 = open("/home/%s/Desktop/variable/cam3_yoffset" % name, 'r+t')
            cam1offy = f2.read()
            print("check_cam3offy:", cam1offy)
            f2.close()
        ...
        if os.path.isfile("/home/%s/Desktop/variable/exposure3" % name):
            f = open("/home/%s/Desktop/variable/exposure3" % name, 'r+t')
            exposure1 = f.read()
            print("check_expoure3:", exposure1)
            f.close()
...
if __name__=='__main__':
    ...
    cam1step3(2)
    check(2)
    print("cam3:Full ROI calibration finished")
