...
# 5. Date load Function
    ...
    def data_load(self):
        ...
        self.file_name = '/home/%s/Desktop/Bacometer_Value/CKD/%s_R.csv' % (self.name, self.date)
        if os.path.isfile(self.file_name):
            ...
            self.tableWidget.setHorizontalHeaderLabels(["Date", "Time", "Case", "Vial", "Result"])
...
# 6. Export Function
    ...
    def export_function(self):
        ...
        if os.path.exists('/sys/devices/70090000.xusb/usb1/1-2/1-2.1/'):
            if os.path.exists('/home/%s/Desktop/Bacometer_Value/CKD/%s_R.csv' % (self.name, self.date)) == False:
                Nodata = NodataWindow(parent=self)
                Nodata.show()
                Nodata.exec_()
            else:
                if os.path.isdir('/media/%s/%s/Bacometer_Value/CKD' % (self.name, self.device)) == False:
                    os.system("sudo mkdir /media/%s/%s/Bacometer_Value" % (self.name, self.device))
                    os.system("sudo mkdir /media/%s/%s/Bacometer_Value/CKD" % (self.name, self.device))
#                    self.datevar = self.data_calendar.selectedDate()
#                    self.date = self.datevar.toString("yyyy-MM-dd")
                    os.system("cp -r /home/%s/Desktop/Bacometer_Value/CKD/%s_R.csv /media/%s/%s/Bacometer_Value/CKD/%s_R.csv" % (self.name, self.date, self.name, self.device, self.date))
...
#        self.date = self.datevar.toString("yyyy-MM-dd")
           ...
           if self.Question == 1:           
               os.system("rm -r /home/%s/Desktop/Bacometer_Value/CKD/%s.csv" % (self.name, self.date))
               os.system("rm -r /home/%s/Desktop/Bacometer_Value/CKD/%s_R.csv" % (self.name, self.date))
               os.system("rm -r /home/%s/Desktop/Bacometer_Data_M/CKD/Bacometer_%s/%s/" % (self.name, self.bacono, self.date))
               self.data_load()
